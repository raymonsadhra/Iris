...
from .constants import DEFAULT_TARGET_LABELS
...
class Settings:
    ...
    target_labels: tuple[str, ...] = tuple(sorted(DEFAULT_TARGET_LABELS))
    ...
    close_area_ratio: float = 0.18
    very_close_area_ratio: float = 0.32
...
def get_settings() -> Settings:
    ...
    if target_labels_raw.strip():
        ...
        target_labels = tuple(sorted(DEFAULT_TARGET_LABELS))
    ...
    return Settings(
        ...
        confidence_threshold=_env_float("IRIS_CONFIDENCE", 0.35),
        ...
        command_cooldown_sec=_env_float("IRIS_COOLDOWN_SEC", 1.0),
        enable_closeness=_env_bool("IRIS_ENABLE_CLOSENESS", True),
        close_area_ratio=_env_float("IRIS_CLOSE_AREA_RATIO", 0.18),
        very_close_area_ratio=_env_float("IRIS_VERY_CLOSE_AREA_RATIO", 0.32),
